...
if ($exception instanceof \yii\web\HttpException) {
    ...
    if ($name !== null) {
        echo $handler->htmlEncode($name . ' – ' . $exception::class);
    } else {
        echo $handler->htmlEncode($exception::class);
    }
}
?></title>
...
<body>
    <div class="header">
        <div class="tools">
            <textarea id="clipboard"><?php echo $handler->htmlEncode($exception); ?></textarea>
            ...
            <a href="https://stackoverflow.com/search?<?php echo http_build_query(['q' => $exception->getMessage()]); ?>" title="Search error on Stackoverflow" target="_blank">
            ...
            <a href="https://www.google.com/search?<?php echo http_build_query(['q' => $exception->getMessage()]); ?>" title="Search error on Google" target="_blank">
        ...
        <?php if ($exception instanceof \yii\base\ErrorException) { ?>
            ...
            <h1>
                ...
                &ndash; <?php echo $handler->addTypeLinks($exception::class); ?>
            </h1>
        <?php } else { ?>
            ...
            if ($exception instanceof \yii\web\HttpException) {
                ...
                echo ' &ndash; ' . $handler->addTypeLinks($exception::class);
            } else {
                $name = $handler->getExceptionName($exception);
                if ($name !== null) {
                    ...
                    echo '<span>' . $handler->htmlEncode($exception::class) . '</span>';
                }
            }
            ?></h1>
        <?php } ?>
    ...
    <div class="call-stack">
        <?php echo $handler->renderCallStack($exception); ?>
    </div>
